...
struct tree_node* read_tree_file(const char *filename)
{
	...
	char *entryList = (char*) calloc((size_t) 100, sizeof(char));		//Allocate space for entries in tree file
...
pid_t* forkAndSleep(struct tree_node* root, int fdPipe[])
{
	...
	{
		struct tree_node* currentChild = root->children[i];
		char* buf = (char*) calloc((size_t) 100, sizeof(char*));
		write(fdPipe[1], buf, 100);
		sleep(.1);
	}
	else
	{
		...
		for(i = 0; i < root->numChildren; i++) 							//loop goes through all children of current node
		{
			...
			else if(pid == (pid_t) 0)	//Child process
			{
				...
				write(arrFd[i][1], &buf, 100);
				forkAndSleep(currentChild, arrFd[i]);
				printf("Code does not get here\n");
				exit(0);
			}
			else 						//Parent process
			{
				...
				*/
				...
				read(arrFd[i][0], &buf, 100);
				//wait(NULL);
				printf("Parent %d done waiting. Child has terminated\n", (int) getpid());
			}
			//wait(NULL);
